            ...
            <table className="w-full text-left text-xs font-mono">
              ...
              <tbody className="divide-y divide-slate-800/60 text-[11px]">
                {scenarioOutput.results?.map((r) => (
                  <tr key={r.step} className="hover:bg-slate-800/30">
                    <td className="py-1.5 px-2 text-slate-400">#{r.step}</td>
                    <td className="py-1.5 px-2 text-slate-300 font-sans">{r.action}</td>
                    <td className="py-1.5 px-2 text-blue-400">
                      {r.previous_state} &rarr; <span className="font-bold text-teal-300">{r.next_state}</span>
                    </td>
                    <td className="py-1.5 px-2 text-indigo-300">{r.reason}</td>
                    <td className="py-1.5 px-2 text-slate-400 font-sans">{r.note || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
