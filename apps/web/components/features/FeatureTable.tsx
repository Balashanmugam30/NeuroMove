      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-2xs bg-slate-100 text-slate-700 px-2 py-0.5 rounded font-mono font-bold border border-slate-200">
        ...
        <div className="flex items-center space-x-2">
          ...
          <select
            ...
            className="px-3 py-1.5 text-xs bg-slate-50 border border-slate-300 rounded-lg text-slate-900 font-sans"
          >
          ...
          <button
            ...
            className="inline-flex items-center px-3 py-1.5 text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200 rounded-lg hover:bg-blue-100 transition shadow-2xs"
          >
            Download CSV
          </button>
        </div>
      ...
      <div className="overflow-x-auto border border-slate-200 rounded-lg">
        <table className="w-full text-left text-xs">
          <thead className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200">
            <tr>
              <th className="px-3 py-2.5 whitespace-nowrap font-mono text-2xs uppercase">Trial ID</th>
              <th className="px-3 py-2.5 whitespace-nowrap font-mono text-2xs uppercase">Subject</th>
              <th className="px-3 py-2.5 whitespace-nowrap font-mono text-2xs uppercase">Class Label</th>
              {featureNames.map((feat) => (
                <th key={feat} className="px-3 py-2.5 whitespace-nowrap font-mono text-2xs uppercase">
                  {feat}
                </th>
              ))}
            </tr>
            ...
            {filteredRows.map((row, idx) => (
              ...
              >
                ...
                <td className="px-3 py-2 whitespace-nowrap">
                  <span className="inline-block px-2 py-0.5 rounded text-2xs font-bold bg-blue-50 text-blue-700 border border-blue-200">
                    {row.label}
                  </span>
                </td>
                {featureNames.map((feat) => (
                  <td
                    key={feat}
                    className="px-3 py-2 font-mono text-slate-700 whitespace-nowrap text-right"
                  >
